...
describe('POST /auth/register', () => {
  ...
  it('returns 400 for duplicate email', async () => {
    ...
    const res = await request(app).post('/api/auth/register').send({ email, password: 'other' });
    expect(res.status).toBe(400);
    expect((res.body as { message?: string }).message).toMatch(/already registered/i);
  });

  it('returns 400 when email or password missing', async () => {
    const res1 = await request(app).post('/api/auth/register').send({ password: 'x' });
    expect(res1.status).toBe(400);
    const res2 = await request(app).post('/api/auth/register').send({ email: 'a@b.co' });
...
describe('POST /auth/login', () => {
  it('returns 200 with token for valid credentials', async () => {
    const email = uniqueEmail();
    await request(app).post('/api/auth/register').send({ email, password: 'secret' });
  ...
  it('returns 401 for wrong password', async () => {
    const email = uniqueEmail();
    await request(app).post('/api/auth/register').send({ email, password: 'correct' });
...
describe('GET /auth/me (protected)', () => {
  ...
  it('returns 200 with userId when token is valid', async () => {
    const email = uniqueEmail();
    const reg = await request(app).post('/api/auth/register').send({ email, password: 'p' });
